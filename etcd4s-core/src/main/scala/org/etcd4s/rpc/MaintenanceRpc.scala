...
private[etcd4s] class MaintenanceRpc(protected val stub: MaintenanceStub) extends Maintenance {
  ...
  override def defragment(request: DefragmentRequest): Future[DefragmentResponse] = stub.defragment(request)
  ...
  override def snapshot(request: SnapshotRequest, responseObserver: StreamObserver[SnapshotResponse]) =
    stub.snapshot(request, responseObserver)

  override def moveLeader(request: MoveLeaderRequest): Future[MoveLeaderResponse] = stub.moveLeader(request)
}
